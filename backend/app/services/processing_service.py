...
from app.core.exceptions import BasalamError, OpenRouterError, AppException  # noqa: F401
...
# ---------------------------------------------------------------------------
# Module-private helpers (preserved verbatim from processing.py)
# ---------------------------------------------------------------------------
...
class ProcessingService(LoggerMixin):
    """Orchestrates the full AI-analyse → price-suggest → Basalam-submit pipeline."""
    ...
    async def process_product_job(self, product_id: UUID) -> None:
        ...
        if product is None:
            self.logger.error("process_product_job: product not found product_id=%s", product_id)
            return
        ...
        if oauth is None:
            self.logger.warning(
                "process_product_job: no oauth account product_id=%s user_id=%s", product_id, user.id
            )
            product.status = ProductStatus.FAILED
            product.errors = {"message": "no basalam token"}
            if product.withdraw_tx_id:
                await self.ledger.reverse_transaction(product.withdraw_tx_id)
            await db.flush()
            await db.commit()
            return

        if not images:
            self.logger.warning("process_product_job: no images product_id=%s", product_id)
            product.status = ProductStatus.FAILED
        ...
        self.logger.debug("process_product_job: token decrypted product_id=%s", product_id)
        ...
        try:
            ...
            # --------------------------------------------------------------
            self.logger.info("process_product_job: fetching categories product_id=%s", product_id)
            ...
            # --------------------------------------------------------------
            # Step 5: Enhance images — only those the user opted in for AI
            # (use_enhanced=True) and that don't already have an enhanced_url.
            # --------------------------------------------------------------
            for img in images:
                if img.enhanced_url:
                    continue
                if not img.use_enhanced:
                    continue
                ...
                try:
                    result = await self.openrouter.enhance_product_image(
                ...
                if cat_id_int is not None:
                    ...
                    if matched_cat:
                        product.category_title = (
                            matched_cat.get("path")
                            or matched_cat.get("title")
                            or category_info.get("title")
                            or ""
                        )
                    else:
            ...
            if isinstance(weight_raw, dict) and product.weight is None:
                ...
                if weight_conf >= 0.5:
                    ...
                    if weight_grams is not None:
                        ...
                        self.logger.debug(
                            "process_product_job: set weight=%s product_id=%s",
                            product.weight,
                            product_id,
                        )
            ...
            for guess in guesses:
                ...
                if matched_attr is not None:
                    ...
                    self.logger.debug(
                        "process_product_job: set attribute attr_id=%s product_id=%s",
                        attr_key,
                        product_id,
                    )
            ...
            if product.preparation_days is None:
                field_errors.setdefault("preparation_days", []).append(
                    "زمان آماده‌سازی وارد نشده است."
                )
            ...
            for attr in flat_attrs:
                is_required = bool(
                    attr.get("required") or attr.get("is_required") or attr.get("isRequired")
                )
                if is_required:
            ...
            # --------------------------------------------------------------
            self.logger.info("process_product_job: uploading images product_id=%s", product_id)
            photo_ids: list[int] = []
            skipped_unprocessed = 0
            for img in images:
                # Only the AI-enhanced version is sent to Basalam. If AI enhance
                # failed for this image (no enhanced_url or user opted out via
                # use_enhanced=False), skip it — the raw user upload must not be
                # published.
                if not (img.use_enhanced and img.enhanced_url):
                    skipped_unprocessed += 1
                    self.logger.warning(
                        "process_product_job: skipping non-enhanced image img_id=%s use_enhanced=%s has_enhanced=%s",
                        img.id,
                        img.use_enhanced,
                        bool(img.enhanced_url),
                    )
                    continue
                upload_result = await client.upload_product_photo(
                    image_data_url=img.enhanced_url,
                    filename=img.filename or "product.jpg",
                )
                ...
                if photo_id is not None:
                    ...
                    self.logger.debug(
                        "process_product_job: uploaded photo photo_id=%s img_id=%s",
                        photo_id,
                        img.id,
                    )
                else:
            ...
            if not photo_ids:
                if skipped_unprocessed > 0:
                    raise BasalamError(
                        "هیچ عکسی بعد از پردازش AI آماده ارسال نبود.", 422
                    )
                raise BasalamError("هیچ عکسی با موفقیت آپلود نشد.", 422)
            if skipped_unprocessed > 0:
                self.logger.warning(
                    "process_product_job: %d image(s) skipped (no AI enhancement) product_id=%s",
                    skipped_unprocessed,
                    product_id,
                )
            ...
            # Enrich payload from AI analysis where it is confident enough.
            ai_keywords = analysis.get("keywords") if isinstance(analysis, dict) else None
            if isinstance(ai_keywords, list) and ai_keywords:
                product_dict["keywords"] = [str(k).strip() for k in ai_keywords if str(k).strip()]

            ai_dims = analysis.get("dimensions") if isinstance(analysis, dict) else None
            if isinstance(ai_dims, dict):
                length = ai_dims.get("length_cm")
                width = ai_dims.get("width_cm")
                height = ai_dims.get("height_cm")
                conf = ai_dims.get("confidence", 0)
                try:
                    conf_num = float(conf)
                except (TypeError, ValueError):
                    conf_num = 0.0
